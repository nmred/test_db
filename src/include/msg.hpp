...
int msgExtractDelete(char **ppBuffer, bson::BSONObj &key);

int msgBuildQuery(char **ppBuffer, int *pBufferSize, bson::BSONObj &key);

int msgExtractQuery(char **ppBuffer, bson::BSONObj &key);
...
int msgExtractCommand(char *pBuffer, int &numInsert, const char **ppObjStart);
